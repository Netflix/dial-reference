...
function test() {
    var host = argv.host;
    var key = "TEST";
    return dial.sleepSystem(host, key)
        .then(function (response) {
            utils.printTestInfo(__filename.slice(__dirname.length + 1), "Sleep the system using DIAL server and key is required and is provided. Expect response code 200.");
            return response
        })
        .then(function (response) {

            if (response.statusCode !== 200) {
                throw new Error(`Error sleeping system while providing required key.  Expected to receive a 200 from the DIAL server but instead received ${response.statusCode}`);
            }
        })
        .then(function () {
